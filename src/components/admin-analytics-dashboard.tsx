    ...
    <section aria-label="Key metrics" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {metrics.map(({ label, value, hint, Icon }, i) => <div key={label} className={`${panel} ${i === 3 ? 'bg-gradient-to-br from-primary/10 via-card to-violet-500/5' : ''}`}>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-2">
          ...
          {i === 0 ? <span aria-live="polite" title={`New registrations: ${comparison.current} vs ${comparison.previous}. This compares signups, not the all-time total.`} className={`inline-flex shrink-0 items-center gap-1.5 rounded-md px-2 py-1 text-xs font-semibold tabular-nums ${comparison.current > comparison.previous ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : comparison.current < comparison.previous ? 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400' : 'bg-muted text-muted-foreground'}`}>
            <span className="sr-only">New registrations: {comparison.current > comparison.previous ? 'increase' : comparison.current < comparison.previous ? 'decrease' : 'no change'} </span>
            {comparison.current !== comparison.previous && <svg aria-hidden="true" width="10" height="6" viewBox="0 0 10 6" className={comparison.current < comparison.previous ? 'rotate-180' : ''}><path d="M5 0 10 6H0Z" fill="currentColor" /></svg>}
            {comparison.percent === null ? 'New' : `${Math.abs(comparison.percent).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}%`}
